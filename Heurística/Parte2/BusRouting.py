...
class Simulador():
    ...
    if len(sys.argv) != 3 or sys.argv[1][-5:] != ".prob" or not sys.argv[2].isdigit() or int(sys.argv[2]) not in [0, 1, 2]:
        print("Correct use: python3 BusRouting.py *.prob n")
        print("Where n:\nn=0 -> no heuristic, breadth first\nn=1 -> Number of misplaced kids\nn=2 -> 2nd heuristic: Remaining kids * Seats in theRemaining kids * Seats in the bus")
    ...
    #---Ejecucion---#
        ...
        file.write("Coste total: " + str(node.f) + '\n')
